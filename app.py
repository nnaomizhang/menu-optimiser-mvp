    ...
    for i, item in enumerate(st.session_state["manual_items"]):
        ...
        with col5:
            st.session_state["manual_items"][i]["monthly_units_sold"] = st.number_input(
                ...
                key=f"units_{i}",
                help="How many of this item you sell in a typical month. An estimate is fine.",
                label_visibility="visible" if i == 0 else "collapsed"
            )
        with col6:
...
<p style="font-size:0.8rem; color:#7A7060; font-family:'DM Sans',sans-serif; 
   ...
   <strong>Advisory only.</strong> MenuMind recommendations are generated by AI and should 
   be treated as a starting point for decision-making. Always apply your own judgment, 
   knowledge of your customers, and business context before making any pricing changes.
</p>
""", unsafe_allow_html=True)
...
- Marginal: recommend removal, action = Remove. For removed items, recommended_price must equal current_price — do not suggest a price change. Calculate the gross profit loss (gross margin per unit x monthly units sold) and show it honestly e.g. "-£60/month gross profit" but always follow it with "likely offset by customer substitution to higher margin items" so the owner understands the net impact is probably neutral or better. Do not blindly say neutral to positive without showing the actual calculation.
